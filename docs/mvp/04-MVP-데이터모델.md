...
- 기본 설정은 인증된 사용자 기준 서버 데이터로 생성하고, 모바일 로컬 DB는 생성된 설정을 캐시한다.
...
권장 구현은 Expo SQLite 또는 동등한 모바일 로컬 DB다. 로컬 DB 초기화는 스키마와 캐시 준비만 담당하며, 예시 카테고리를 실제 사용자 데이터로 주입하지 않는다.
...
- 모바일 앱은 서버에서 생성된 기본 설정과 사용자가 만든 카테고리를 로컬 DB에 캐시한다.
