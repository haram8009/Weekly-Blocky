...
이 문서는 MVP에서 사용할 데이터 구조, 저장 규칙, 서버 공유 규칙, 사진 매칭 규칙을 정의한다.

## 2. 데이터 저장 원칙

- MVP 기록 CRUD는 Supabase-first로 구현한다.
- 서버는 모바일 앱과 데스크톱 웹이 같은 데이터를 보기 위한 원본 저장소 역할을 한다.
- 모바일 앱과 데스크톱 웹은 서버에 저장된 데이터를 열람한다.
- 기본 설정은 인증된 사용자 기준 서버 데이터로 생성하고, 모바일과 웹이 이를 조회한다.
- 오프라인 로컬 기록 저장과 복구 후 동기화는 MVP 이후 후속 범위로 둔다.
- 카테고리는 사용자가 직접 생성한 사용자 소유 데이터만 저장한다.
- 예시 카테고리는 최초 생성 힌트로만 제공하며, 서버나 로컬 `categories` 테이블에 자동 저장하지 않는다.
- 사진 원본은 자동 업로드하지 않는다.
...
- ID는 서버 또는 클라이언트에서 생성할 수 있다.
- MVP는 Supabase 저장 성공을 기준으로 데이터를 확정한다.
- 오프라인 생성 후 서버 동기화는 후속 범위로 둔다.
...
| Category | 시간 기록 분류 | 서버 |
| TimeEntry | 실제 시간 사용 기록 | 서버 |
| Template | 반복 루틴 묶음 | 후속 |
| TemplateEntry | 템플릿 안의 시간 블록 | 후속 |
| WeekReview | 주간 회고 | 서버 |
| PhotoReference | 모바일 사진첩 사진 참조 | 로컬 중심, 일부 서버 |
| AppSettings | 사용자 설정 | 서버 |
| SyncState | 모바일 동기화 상태 | 후속 로컬 |
...
모바일 앱의 로컬 변경 사항과 서버 동기화 상태를 관리한다. MVP에서는 구현하지 않고, 오프라인 로컬 기록 저장을 추가할 때 후속으로 사용한다.
...
- 후속 오프라인 기록에서 모바일 앱에 오프라인 변경이 생기면 `pending` 상태로 저장한다.
...
MVP의 기록, 카테고리, 회고, 설정은 로컬 DB가 아니라 Supabase를 원본으로 사용한다. 모바일 로컬 저장은 사진 참조, 생성된 썸네일 파일, 인증 세션, 후속 오프라인 확장 기반에 한정한다.
...
오프라인 기록을 후속으로 구현할 때는 Expo SQLite 또는 동등한 모바일 로컬 DB를 사용한다. 로컬 DB 초기화는 스키마와 캐시 준비만 담당하며, 예시 카테고리를 실제 사용자 데이터로 주입하지 않는다.

### 서버

서버는 계정 기반 데이터 저장과 데스크톱 웹 열람을 위해 사용한다.
...
- 신규 사용자의 회원가입 또는 최초 로그인 후 기본 설정을 사용자 소유 서버 데이터로 생성한다.
- 모바일 앱과 데스크톱 웹은 서버에서 생성된 기본 설정과 사용자가 만든 카테고리를 조회한다.
- 기본 설정 초기화는 여러 번 실행되어도 사용자별 중복 데이터가 생기지 않아야 한다.
- 기본 설정과 예시 카테고리의 기준 값은 `packages/domain/src/defaults.ts`에 둔다.
- 기본 설정은 `settings:{userId}` 형식의 안정적인 ID와 `userId` 유니크 제약으로 중복 생성을 막는다.
- 예시 카테고리는 `id`, `userId`, `createdAt`, `updatedAt`을 갖지 않는다. 실제 `Category`는 사용자가 저장할 때 생성한다.
- 로컬 개발용 샘플 데이터가 필요하면 예시 카테고리 표시 흐름과 별도 경로로 주입한다.
